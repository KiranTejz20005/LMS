...
```typescript
import { apiClient, safeRequest, gurukulx } from '$lib/utils/services/api';
...
// Use with Hono RPC
const data = await gurukulx.users.$get();
const user = await gurukulx.users.$post({ json: { name: 'John' } });
```
