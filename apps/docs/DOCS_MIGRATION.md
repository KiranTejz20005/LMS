# Migrating Docs from docs.GurukulX.com to GurukulX.com/docs

## Summary

This guide documents the configuration to serve the docs at `GurukulX.com/docs` instead of `docs.GurukulX.com` using Vercel rewrites with TanStack Start.
...
### 2. Main Site Configuration (`apps/GurukulX-com`)

**Created `apps/GurukulX-com/vercel.json`**:
```json
{
  "rewrites": [
    {
...
This proxies all `/docs/*` requests from `GurukulX.com` to your docs deployment.

### 3. Cleanup

- Removed empty `apps/GurukulX-com/src/routes/(redirects)/docs/` folder that would conflict with the Vercel rewrite

## How It Works

1. **User visits:** `GurukulX.com/docs/home`
...
5. **User sees:** Content at `GurukulX.com/docs/home` (URL unchanged)
...
1. Make sure `apps/GurukulX-com/vercel.json` is committed
2. Deploy or redeploy to `GurukulX.com`

**Vercel Project Settings for Main Site:**
- **Root Directory:** `apps/GurukulX-com`
- **Build Command:** `pnpm build`
...
1. ✅ Visit `GurukulX.com/docs` - should show docs homepage
2. ✅ Visit `GurukulX.com/docs/quickstart/signup` - should show specific doc pages
...
```
User Request: GurukulX.com/docs/quickstart/signup
       ↓
Vercel (GurukulX.com) - reads vercel.json
       ↓
...
Serves: /quickstart/signup route
       ↓
User sees content at: GurukulX.com/docs/quickstart/signup
```
